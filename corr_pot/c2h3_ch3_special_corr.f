      real*8 function c2h3_ch3_special_corr(natoms,x,rparm,iparm)
c
...
c
     ...
     x             (x(3,nb)-x(3,na))**2 )
      ...
      call cc_000_p15(rcc,c2h3_ch3_special_corr)
...
c
      ...
      r52 = 0.0d0
      do idim = 1 , 3
         r1(idim) = x(idim,6) - x(idim,1) 
         r2(idim) = x(idim,1) - x(idim,3)
         r3(idim) = x(idim,3) - x(idim,4)
         r4(idim) = x(idim,6) - x(idim,3)
         r5(idim) = x(idim,1) - x(idim,4)
         r12 = r12 + r1(idim)**2
         r22 = r22 + r2(idim)**2
         r32 = r32 + r3(idim)**2
         r42 = r42 + r4(idim)**2
     ...
     x     write (6,*) 'error in the2, cthe2 = ',cthe2
      ...
      pi2 = asin(1.0d0)
      if (rparm.gt.0.5d0) then
         if ((the1.gt.pi2).or.(tau1.gt.pi2)) c2h3_ch3_special_corr=
     $    1.0d20
      else
         if ((rparm.lt.-0.5d0).and.(rparm.gt.-1.5d0)) then 
            if ((the1.lt.pi2).or.(tau1.lt.pi2)) c2h3_ch3_special_corr=
     $     1.0d20
         else
            if (rparm.lt.-1.5d0) then 
               if (the1.lt.pi2) c2h3_ch3_special_corr=1.0d20
            endif
         endif
      endif
